class ItcCustomSelect {
  static EL = 'itc-select6';
  static EL_SHOW = 'itc-select6_show';
  static EL_OPTION = 'itc-select6__option';
  static EL_OPTION_SELECTED = 'itc-select6__option_selected';
  ...
  static template(params) {
    ...
    options.forEach((option, index) => {
      ...
      items.push(`<li class="itc-select6__option${selectedClass}" data-select="option"
        data-value="${option[0]}" data-index="${index}">${option[1]}</li>`);
    });
    return `<button type="button" class="itc-select6__toggle" name="${name}"
      value="${selectedValue}" data-select="toggle" data-index="${selectedIndex}">
      ${selectedContent}</button><div class="itc-select6__dropdown">
      <ul class="itc-select6__options">${items.join('')}</ul></div>`;
  }
