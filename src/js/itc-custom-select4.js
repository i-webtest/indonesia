class ItcCustomSelect {
  static EL = 'itc-select5';
  static EL_SHOW = 'itc-select5_show';
  static EL_OPTION = 'itc-select5__option';
  static EL_OPTION_SELECTED = 'itc-select5__option_selected';
  ...
  static template(params) {
    ...
    options.forEach((option, index) => {
      ...
      items.push(`<li class="itc-select5__option${selectedClass}" data-select="option"
        data-value="${option[0]}" data-index="${index}">${option[1]}</li>`);
    });
    return `<button type="button" class="itc-select5__toggle" name="${name}"
      value="${selectedValue}" data-select="toggle" data-index="${selectedIndex}">
      ${selectedContent}</button><div class="itc-select5__dropdown">
      <ul class="itc-select5__options">${items.join('')}</ul></div>`;
  }
